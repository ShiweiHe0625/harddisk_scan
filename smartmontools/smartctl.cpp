...
#include <Windows.h>
static const WCHAR *kSmartmontoolsScanMutexName = L"SmartmontoolsScanMutex";
static const WCHAR *kSmartmontoolsScanRet = L"SmartmontoolsScanRet";
static const int kSmartmontoolsScanMaxFileSize = 655360;  // 640KB

struct SmartLock {
  HANDLE mutex;

  explicit SmartLock(HANDLE _mutex) : mutex(_mutex) {
    WaitForSingleObject(mutex, INFINITE);
  }

  ~SmartLock() { ReleaseMutex(mutex); }
};
...
class ShareString {
 public:
  ...
  ~ShareString() {
    SAFE_CLOSE(file_mapping_);
    SAFE_CLOSE(mutex_);
  }

  void Init(const WCHAR *map_name, const WCHAR *mutex_name, const int map_size) {
    if (init_) return;
    mutex_ = CreateMutexW(NULL, FALSE, mutex_name);
    SmartLock lock(mutex_);
    map_size_ = map_size;
    file_mapping_ =
        OpenFileMappingW(FILE_MAP_READ | FILE_MAP_WRITE, FALSE, map_name);
    if (!file_mapping_) {
      jerr("OpenFileMapping failed. %d", GetLastError());
      return;
    } else {
      info_ = reinterpret_cast<char *>(MapViewOfFile(
          file_mapping_, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, 0));
      init_ = TRUE;
    }
  }
  ...
  void Set(const std::string &info) {
    SmartLock lock(mutex_);
    memset(info_, 0, map_size_);
    // 写入当前值
    memcpy(info_, info.c_str(), info.size());
  }
...
int main(int argc, char **argv)
{
  ...
  ShareString share_str;
  share_str.Init(kSmartmontoolsScanRet, kSmartmontoolsScanMutexName,
                 kSmartmontoolsScanMaxFileSize);
  share_str.Set(jglb.to_string());
  return status;
}
...
//   // 开启json 并扫描
//    int temp_argc = 3;
//    char **temp_argv = new char *[temp_argc + 1] { "", "--j", "--scan" };
//    int ret = main(temp_argc, temp_argv);
//    delete[] temp_argv;
//  
//    std::string ret_str = jglb.to_string();
//    jglb.clear();
//    JsonCppObj root;
//    if (root.parse(ret_str)) {
//      int dev_size = root.getArraySize("devices");
//      for (int i = 0; i < dev_size; ++i) {
//        std::string dev_name =
//            root.getStringEx(JsonCppObj::genPath("devices[%d].name", i));
//        std::string dev_type =
//            root.getStringEx(JsonCppObj::genPath("devices[%d].type", i));
//        if (dev_name.empty()) continue;
//        // 重置命令行参数解析位置的缓存
//        optind = 1;
//        // 构造获取硬盘详情的命令行参数
//        temp_argc = 5;
//        char *parm_type = new char[dev_type.size() + 1];
//        char *parm_name = new char[dev_name.size() + 1];
//        strcpy(parm_type, dev_type.c_str());
//        strcpy(parm_name, dev_name.c_str());
//        temp_argv =
//            new char *[temp_argc + 1] { "", "-d", parm_type, "-x", parm_name };
//        ret = main(temp_argc, temp_argv);
//        delete parm_type;
//        delete parm_name;
//        delete[] temp_argv;
//        ret_str = jglb.to_string();
//        jglb.clear();
//      }
//   }
// 
