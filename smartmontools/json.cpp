...
std::string json::to_string() {
  ss_.str("");
  ss_.clear();
  if (m_root_node.type != nt_unset) to_string(&m_root_node);
  return ss_.str();
}

void json::clear() {
  for (size_t i = 0; i < m_root_node.childs.size(); i++)
    delete m_root_node.childs[i];
  m_root_node.childs.clear();
  m_root_node.key2index.clear();
}

void json::to_string(const node *p) {
  if (!p->key.empty()) ss_ << R"(")" << p->key << R"(":)";
  switch (p->type) {
    case nt_object:
    case nt_array:
      ss_ << (p->type == nt_object ? "{" : "[");
      if (!p->childs.empty()) {
        bool first = true;
        for (node::const_iterator it(p, false); !it.at_end(); ++it) {
          if (!first) ss_ << ",";
          const node *p2 = *it;
          if (!p2) {
            // Unset element of sparse array
    ...
    case nt_uint128: {
      char buf[64];
      ss_ << R"(")"
          << std::string(uint128_hilo_to_str(buf, p->intval_hi, p->intval))
          << R"(")";
    } break;
    case nt_string:
      ss_ << R"(")" << p->strval << R"(")";
